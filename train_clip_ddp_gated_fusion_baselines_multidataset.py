#!/usr/bin/env python3
"""
train_clip_ddp_gated_fusion_baselines_multidataset.py

One script that trains CLIP heads + (optional) sensor BEV heads with a gated fusion module.

Supports:
  ✅ nuScenes Radar/LiDAR pointcloud files -> BEV on-the-fly
  ✅ Waymo v2 cached LiDAR BEV (.npy) referenced by token strings in CSV

Expected CSV columns (minimum):
  - image_path
  - label_vehicle_in_front
  - lidar_path or radar_path (optional depending on --sensor / --sensor_col)

Waymo v2 convention (from your CSV generator):
  lidar_path token example:
    waymo_v2|training|14705303724557273004_3105_000_3125_000|1521755388022264|0

This script will load cached BEV from --waymo_bev_dir using default template:
  {waymo_bev_dir}/{split}/{segment}/{ts}_{laser}.npy
and fallback variants.

Example (Waymo):
  torchrun --nproc_per_node=4 train_clip_ddp_gated_fusion_baselines_multidataset.py \
    --train_csv train_front_vehicle_waymo_v2.csv --val_csv val_front_vehicle_waymo_v2.csv \
    --sensor lidar --sensor_col lidar_path --waymo_bev_dir /mnt/raid0/waymo_bev \
    --train_mode joint --epochs 10 --batch_size 128 --lr 1e-3 \
    --tune_threshold --tune_threshold_last_only --tune_heads cam,sensor,fusion,late --tune_metric f1 \
    --gate_q_mode density --save_ckpt checkpoints/waymo_best.pt --save_json runs/waymo.json
"""
...
# nuScenes point clouds (optional; only needed if you load nuScenes radar/lidar files)
try:
...
# ============================================================
# Image corruptions (optional robustness story)
# ============================================================
...
# ============================================================
# BEV helpers (for nuScenes on-the-fly)
# ============================================================
...
# ============================================================
# Waymo cached BEV helpers
# ============================================================
def is_waymo_v2_token(s: str) -> bool:
    return isinstance(s, str) and s.startswith("waymo_v2|")

def parse_waymo_v2_lidar_token(s: str):
    """
    waymo_v2|split|segment|ts|laser
    """
    parts = str(s).split("|")
    if len(parts) != 5:
        return None
    if parts[0] != "waymo_v2":
        return None
    _, split, segment, ts, laser = parts
    try:
        return split, segment, int(ts), int(laser)
    except Exception:
        return None
...
def waymo_cached_bev_candidates(token: str, waymo_bev_dir: str, template: Optional[str] = None) -> List[str]:
    parsed = parse_waymo_v2_lidar_token(token)
    if parsed is None:
        return []
    split, segment, ts, laser = parsed

    # Default template if not provided
    if template is None:
        template = "{bev}/{split}/{segment}/{ts}_{laser}.npy"

    # Primary + common fallbacks
    cand = [
        template.format(bev=waymo_bev_dir, split=split, segment=segment, ts=ts, laser=laser),
        os.path.join(waymo_bev_dir, split, segment, f"{ts}.npy"),
        os.path.join(waymo_bev_dir, split, segment, f"{ts}-{laser}.npy"),
        os.path.join(waymo_bev_dir, split, segment, f"{ts}_{laser}_bev.npy"),
    ]

    # Light glob fallback (only if directory exists)
    seg_dir = os.path.join(waymo_bev_dir, split, segment)
    if os.path.isdir(seg_dir):
        g1 = _glob_first(os.path.join(seg_dir, f"{ts}*{laser}*.npy"))
        g2 = _glob_first(os.path.join(seg_dir, f"{ts}*.npy"))
        if g1:
            cand.append(g1)
        if g2:
            cand.append(g2)
...
def load_waymo_cached_bev_or_zeros(
    token: str,
    waymo_bev_dir: str,
    H: int,
    W: int,
    ...
    cand = waymo_cached_bev_candidates(token, waymo_bev_dir, template=template)
    npy_path = None
    for p in cand:
        if os.path.isfile(p):
            npy_path = p
    ...
    if npy_path is None:
        if is_main():
            print(f"[Waymo BEV] cache missing for token={token}. Tried e.g.: {cand[:2]} -> zeros", flush=True)
        return torch.zeros((1, H, W), dtype=torch.float32)

    arr = np.load(npy_path)
    if arr.ndim == 2:
        arr = arr[None, ...]
    if arr.ndim != 3 or arr.shape[0] != 1 or arr.shape[1] != H or arr.shape[2] != W:
        if is_main():
            print(f"[Waymo BEV] shape mismatch {arr.shape} for {npy_path}, expected (1,{H},{W}) -> zeros", flush=True)
        return torch.zeros((1, H, W), dtype=torch.float32)

    arr = arr.astype(np.float32)
    # Safe clamp in case cache isn't normalized
    arr = np.clip(arr, 0.0, 1.0)
    return torch.from_numpy(arr)


# ============================================================
# Sensor loaders (nuScenes: pointcloud -> BEV, Waymo: cached BEV)
# ============================================================
...
def load_radar_tensor(path, x_min, x_max, y_min, y_max, H, W, norm_mode, p99, point_drop_p=0.0) -> torch.Tensor:
    if path is None or str(path) == "" or (isinstance(path, float) and math.isnan(path)):
        return torch.zeros((1, H, W), dtype=torch.float32)
...
def load_lidar_tensor(
    ...
    z_min=-3.0, z_max=3.0,
    point_drop_p=0.0,
    waymo_bev_dir: Optional[str] = None,
    waymo_bev_template: Optional[str] = None,
) -> torch.Tensor:
    if path is None or str(path) == "" or (isinstance(path, float) and math.isnan(path)):
        return torch.zeros((1, H, W), dtype=torch.float32)
    ...
    # ---- Waymo cached BEV path ----
    if waymo_bev_dir and is_waymo_v2_token(p):
        return load_waymo_cached_bev_or_zeros(p, waymo_bev_dir, H, W, template=waymo_bev_template)
    # -------------------------------

    # ---- nuScenes pointcloud fallback ----
    ...
    try:
        pc = LidarPointCloud.from_file(p)
    except Exception as e:
        if is_main():
            print(f"[LiDAR] ERROR reading {p}: {e}. Returning zeros.", flush=True)
        return torch.zeros((1, H, W), dtype=torch.float32)
    ...
    if sensor == "lidar":
        def _loader(p: str) -> torch.Tensor:
            return load_lidar_tensor(
                ...
                point_drop_p=point_drop_p,
                waymo_bev_dir=cfg.get("waymo_bev_dir", None),
                waymo_bev_template=cfg.get("waymo_bev_template", None),
            )
        return _loader
    ...
    def _verify_paths(self, n: int = 5):
        ...
        for i in range(n):
            ...
            if self.enable_sensor:
                ...
                # For Waymo token we can't isfile() (it's not a path). Loader will warn if cache missing.
                if is_waymo_v2_token(s_path):
                    continue
...
class MultiHeadBinary(nn.Module):
    def __init__(
        ...
        feat_dim: int,
        use_sensor: bool,
        sensor_in_channels: int = 1,
        gate_hidden: int = 256,
        gate_q_mode: str = "density",
    ):
        super().__init__()
    ...
    def forward(
        ...
        if (not self.use_sensor) or (sensor_bev is None):
            dev = vision_feat.device if vision_feat is not None else (sensor_bev.device if sensor_bev is not None else torch.device("cpu"))
            B = vision_feat.size(0) if vision_feat is not None else (sensor_bev.size(0) if sensor_bev is not None else 1)
        ...
        if compute_fusion:
            ...
            q = compute_gate_q(sensor_bev, self.gate_q_mode, dtype=vision_feat.dtype)
...
def set_trainable_by_mode(model: nn.Module, train_mode: str, use_sensor: bool):
    ...
    if tm in ["cam_only", "joint"]:
        for p in model.cam_head.parameters():
            p.requires_grad = True

    if not use_sensor:
        if tm in ["sensor_only", "fusion_only"]:
            raise ValueError(f"train_mode={tm} requires sensor, but use_sensor=False")
        return

    if tm in ["sensor_only", "joint", "fusion_only"]:
        for p in model.sensor_encoder.parameters():
            p.requires_grad = True

    if tm in ["sensor_only", "joint"]:
        for p in model.sensor_head.parameters():
            p.requires_grad = True

    if tm in ["fusion_only", "joint"]:
        for p in model.fuser.parameters():
            p.requires_grad = True
        for p in model.fusion_head.parameters():
            p.requires_grad = True

    if tm not in ["cam_only", "sensor_only", "fusion_only", "joint"]:
        raise ValueError(f"Unknown train_mode={train_mode}")


# ============================================================
# Metrics (binary confusion matrix)
# ============================================================
...
# ============================================================
# Which heads make sense by mode
# ============================================================
...
# ============================================================
# Eval per head (distributed-safe)
# ============================================================
@torch.no_grad()
...
def main():
    ...
    # Waymo cached BEV support (only used when lidar_path is a token like waymo_v2|...)
    parser.add_argument("--waymo_bev_dir", type=str, default=None)
    parser.add_argument(
        "--waymo_bev_template",
        type=str,
        default=None,
        help="Optional template for cached BEV path. Example: '{bev}/{split}/{segment}/{ts}_{laser}.npy'",
    )
    ...
    parser.add_argument("--eval_only", action="store_true", help="Only run validation eval then exit.")
    parser.add_argument("--load_ckpt", type=str, default=None, help="Path to checkpoint to load for eval_only.")
    ...
    if is_main():
        print(
            ...
            f"waymo_bev_dir={args.waymo_bev_dir}",
            flush=True,
        )
    ...
    cfg = {
        ...
        "waymo_bev_dir": args.waymo_bev_dir,
        "waymo_bev_template": args.waymo_bev_template,
    }
    ...
    train_img_tf = T.Compose([RandomDegrade(args.train_degrade_prob, args.train_degrade_max_sev), preprocess]) \
        if args.train_degrade_prob > 0.0 else preprocess
    ...
    # ---- thresholds default (ALWAYS define before using)
    ...
    # ---- load checkpoint (weights + thresholds)
    if args.load_ckpt is not None:
        ck = torch.load(args.load_ckpt, map_location="cpu")
        if is_main():
            print(f"[Main] Loaded checkpoint: {args.load_ckpt}", flush=True)

        # 1) load model weights into the *underlying module*
        target = model_heads.module if hasattr(model_heads, "module") else model_heads
        state = ck.get("heads_state", ck)

        missing, unexpected = target.load_state_dict(state, strict=False)
        if is_main():
            print(f"[Main] ckpt load: missing={len(missing)} unexpected={len(unexpected)}", flush=True)
            if len(missing) > 0:
                print("  missing (first 10):", missing[:10], flush=True)
            if len(unexpected) > 0:
                print("  unexpected (first 10):", unexpected[:10], flush=True)

        # If missing/unexpected is large, your weights are NOT actually being applied.
        # Ideally these should both be 0.

        # 2) load thresholds from checkpoint (so eval uses tuned values)
        ck_thr = ck.get("thresholds", None)
        if isinstance(ck_thr, dict):
            for k, v in ck_thr.items():
                if k in thresholds:
                    thresholds[k] = float(v)
            if is_main():
                print(f"[Main] thresholds from ckpt: {thresholds}", flush=True)
    ...
    if args.eval_only:
        ...
        if is_main():
            print("\n=== EVAL ONLY RESULTS (VAL) ===")
            for hk in mode_heads:
                r = evals[hk]
                extra = ""
                if hk == "fusion" and r.get("gate_mean", None) is not None:
                    extra = f" gate_mean={r['gate_mean']:.4f}"
                print(f"{hk.upper():5s}: acc={r['acc']:.4f} bal={r['bal_acc']:.4f} "
                      f"f1+={r['f1_pos']:.4f} thr={thresholds[hk]:.2f}{extra}")

            if use_sensor and ("fusion" in mode_heads):
                r = evals["fusion"]
                print(f"VAL BEV stats: empty={r['sensor_empty_frac']:.3f} "
                      f"mean_nnz={r['sensor_mean_nnz']:.1f} mean_bev_sum={r['sensor_mean_bev_sum']:.2f}")
        # ---- SAVE JSON for eval_only (FIX) ----
        if args.save_json is not None and is_main():
            ...
            print(f"[rank 0] saved eval_only summary to {args.save_json}", flush=True)
        ddp_cleanup()
        return
    ...
    # Optional robustness eval (only makes sense if fusion exists)
    if (not args.no_robustness) and use_sensor and ("fusion" in mode_heads) and ("cam" in mode_heads):
        ...
        thr_sen = float(best_thresholds.get("sensor", 0.5))  # <-- add this
        ...
        for s in [1, 2, 3, 4, 5]:
            ...
            r_sen = None
            if "sensor" in mode_heads:
                r_sen = eval_head_binary_distributed(
                    clip_model, model_heads, val_loader_deg, device,
                    thr=thr_sen, head_key="sensor",
                    late_alpha=float(args.late_alpha),
                    has_sensor=True
                )

            if is_main():
                if r_sen is not None:
                    print(
                        f"severity {s}  cam_f1={r_cam['f1_pos']:.4f}  "
                        f"sensor_f1={r_sen['f1_pos']:.4f}  "
                        f"fusion_f1={r_fus['f1_pos']:.4f}  fusion_gate={r_fus['gate_mean']}",
                        flush=True
                    )
                else:
                    print(
            ...
            entry = {"cam": r_cam, "fusion": r_fus}
            if r_sen is not None:
                entry["sensor"] = r_sen
            robustness[f"degrade_{s}"] = entry
        ...
        if "sensor" in mode_heads:
            r_sen = eval_head_binary_distributed(
                clip_model, model_heads, val_loader_cut, device,
                thr=thr_sen, head_key="sensor",
                late_alpha=float(args.late_alpha),
                has_sensor=True
            )
        ...
        if is_main():
            if r_sen is not None:
                print(
                    f"cutout  cam_f1={r_cam['f1_pos']:.4f}  "
                    f"sensor_f1={r_sen['f1_pos']:.4f}  "
                    f"fusion_f1={r_fus['f1_pos']:.4f}",
                    flush=True
                )
            else:
